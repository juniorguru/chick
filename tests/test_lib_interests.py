...
def test_should_notify_when_never_notified():
    ...
    interest: Interest = {"role_id": 42, "last_notified_at": None}

    assert should_notify(interest, now) is True
...
def test_should_not_notify_before_default_cooldown():
    ...
    assert should_notify(interest, now) is False
...
def test_custom_cooldown_respected():
    ...
    assert should_notify(interest, now, cooldown=cooldown) is False
